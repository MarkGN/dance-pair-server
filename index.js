...
const sexes = ["male", "female"]; // aspirational: move to stored in event, so that users can rename as they like
...
app.post("/register", async (req, res) => {
  ...
  try {
    await runTransaction(db, async (t) => {
      ...
      const oppositeSex = body.sex == "male" ? "female" : "male"; // TODO this is ugly
      const potentialPartners = event[oppositeSex+"Registered"];
      var isInvited = false;
      const expiredRegistrants = []
      while (Object.keys(potentialPartners).length) {
        // then take the first registree from there to Invited, put this one onto Invited, and send emails;
        // or rather make this into a while, removing any whose deadline has expired
        const partner = potentialPartners; // TODO .get whichever has the lowest timestamp;
      ...
      if (!isInvited) {
        // set the doc's sex Registered to also include the registrant, with this timestamp
